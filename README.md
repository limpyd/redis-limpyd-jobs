...
- `delayed_until=None`
    Set this to a `datetime` object to set the date on which the job will be really requeued. The real `delayed_until` can also be set by passing the `delayed_for` argument.
...
- `result`
    The data returned by the `execute` method of the worker, which call and return the result of the `run` method of the job (or the `callback` provided to the worker)
...
- `traceback`:
    The traceback at the time of the exception, if the `save_tracebacks` attribute of the worker was set to `True`
...
- `delayed_until=None`
    Set this to a `datetime` object to set the job to be executed in the future. If defined and in the future, the job will be added to the delayed list (a redis sorted-set) instead of the waiting one. The real `delayed_until` can also be set by passing the `delayed_for` argument.

- `delayed_for=None`
    A number of seconds (as a int, float or a `timedelta` object) to wait before adding the job to the waiting list. It will compute the `delayed_until` field of the job.
